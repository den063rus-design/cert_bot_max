...
ALLOWED_EXTENSIONS = {".crt", ".cer", ".pem", ".der"}

# Пороговые дни для плановых уведомлений до окончания срока
ALERT_THRESHOLDS = [60, 30, 14, 7]

# Режим группировки:
# True  -> одно уведомление на организацию (берётся сертификат с самой поздней датой).
# False -> одно уведомление на каждый файл сертификата.
...
# True  -> скрипт работает в цикле (daemon) и проверяет команды в чате.
# False -> одиночный запуск (один проход и выход).
RUN_AS_DAEMON = False

# Интервал полного цикла проверки сертификатов (секунды)
DAEMON_LOOP_INTERVAL = 60

# Интервал опроса команд в чате между полными циклами (секунды)
COMMAND_LOOP_INTERVAL = 3

# Сколько последних сообщений читать из чата за один опрос (1..100)
